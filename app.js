...
function buildSuncodeLayer() {
  ...
  const fixedSvgText = decodeFixedSuncodeSvg();
  const doc = new DOMParser().parseFromString(fixedSvgText, "image/svg+xml");
  const suncodeSvg = doc.documentElement;
  const suncodeViewBox = parseViewBox(fixedSvgText);
...
els.svgInput.addEventListener("change", async (event) => {
  ...
  try {
    ...
    els.statusTitle.textContent = "SVG 读取失败";
    els.statusDetail.textContent = error.message || "请检查上传文件。";
  }
});
...
els.downloadJpeg.addEventListener("click", () => {
  downloadJpeg().catch((error) => {
    ...
    els.statusDetail.textContent = error.message || "请检查 SVG 是否包含浏览器不支持的内容。";
  });
});
